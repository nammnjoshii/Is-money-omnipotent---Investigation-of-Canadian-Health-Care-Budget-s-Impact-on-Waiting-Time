...
```{r install relevant packages, not required if not installed prior}
...
```{r get Budget data, warning = FALSE}
...
```{r get Waiting Time data, warning=FALSE, message=FALSE}
...
```{r cleanData Budget, warning=FALSE, message=FALSE}
...
```{r cleanData Waiting Time, warning=FALSE, message=FALSE}
...
```{r write tables to DB, warning=FALSE, message=FALSE}
dbWriteTable(con,
               "Budget1",
               Budget1,
               row.names = FALSE,
...
```{r join in sql,  warning=FALSE, message=FALSE}
...
``` {r Removing Unwanted Columns from Merged Table,  warning=FALSE, message=FALSE}
...
```{r correlation between Waiting time and budget, warning=FALSE }
...
```{r regression between Budget and Waiting time}
